...
class MtcSender():
    ...
    def __init__(self, fps=25, port=0, portname="SLMTCPort"):
        ...
        libname = Path("/usr/lib/libmtcmaster.so.0")
        if not libname.exists():
            print(f"libmtcmaster.so.0 not found at {libname.parent}, trying development path")
            libname = Path.cwd().parent.parent / "libmtcmaster/libmtcmaster.so"
        if not libname.exists():
            raise FileNotFoundError(f"libmtcmaster.so.0 not found at {libname.parent}")
        print(f"Loading MTC from {libname}")
