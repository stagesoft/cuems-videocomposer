...
const vidout VO[] = {
	...
	{ AV_PIX_FMT_YUV420P, SUP_LIBXV, "XV - X11 video extension",
		...
		&vc_set_fullscreen, &vc_set_ontop, &vc_mousepointer,
		&vc_get_fullscreen, &vc_get_ontop, &vc_letterbox
#else
			NULLOUTPUT
#endif
	},
	{
		...
		&vc_set_fullscreen, &vc_set_ontop, &vc_mousepointer,
		&getfullscreen_null, &getontop_null, &vc_letterbox
#else
			NULLOUTPUT
#endif
	},
...
void render_buffer (uint8_t *mybuffer) {
	...
	if (!splashed) {
		...
		int v0, v1, v2, v3, v4;
		if (vc_get_eq("brightness", &v0)) v0 = 0;
		if (vc_get_eq("contrast", &v1)) v1 = 0;
		if (vc_get_eq("gamma", &v2)) v2 = 0;
		if (vc_get_eq("saturation", &v3)) v3 = 0;
		if (vc_get_eq("hue", &v4)) v4 = 0;
		OBM(brightness, 3);
		OBM(contrast, 23);
		OBM(gamma, 43);
		OBM(saturation, 63);
