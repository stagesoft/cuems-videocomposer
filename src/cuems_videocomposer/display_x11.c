/* cuems-videocomposer - X11/XVideo display
 *
...
void vc_mousepointer (int action) {
...
void vc_get_window_pos (int *rx,  int *ry) {
	Window	dummy;
...
void vc_get_window_size (unsigned int *my_Width, unsigned int *my_Height) {
	int dummyX,dummyY;
...
void vc_resize (unsigned int x, unsigned int y) {
	XResizeWindow(xj_dpy, xj_win, x, y);
}

void vc_position (int x, int y) {
...
#define EQMOD(PROP,STEP) if(!vc_get_eq(PROP,&value)){value+=(STEP); EQCLAMP(value); vc_set_eq(PROP,value); force_redraw=1;}
...
int vc_get_eq(char *prop, int *value) {
#ifdef COLOREQ
# ifdef HAVE_LIBXV
		...
		switch (event.type) {
			...
			case ConfigureNotify:
				if (event.xconfigure.width > 1 && event.xconfigure.height > 1)
				{
					...
					vc_get_window_size(&my_Width,&my_Height);
					xj_dwidth= my_Width;
...
// Menu removed
				...
				break;
			case KeyPress:
				// Keyboard bindings removed - all functionality available via remote control
				// Key events are still notified to remote control for external handling
				{
					KeySym key;
					char buf[100];
...
// Drag and drop removed
	...
	vc_set_fullscreen(xj_fullscreen);
	vc_set_ontop(xj_ontop);
