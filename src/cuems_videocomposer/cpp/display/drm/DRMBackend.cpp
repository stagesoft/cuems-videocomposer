...
bool DRMBackend::initVirtualCanvas() {
    ...
    // Try to load pre-generated display config from the mappings XML.
    // The config is generated by cuems-generate-display-conf (ExecStartPre)
    // and maps connector names to their correct canvas regions, so the
    // layout is correct from the very first frame — no engine re-send needed.
