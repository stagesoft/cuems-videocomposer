...
- **The compositor silently drops OSC layer loads for ~10 s after it starts.** `AsyncVideoLoader`'s worker threads are not running until the display backend has finished its 4-output modeset, and a `/videocomposer/layer/load` that arrives before then is accepted by the OSC socket and goes nowhere — no error, no layer, nothing in the journal to say a load was lost. Any script that restarts the service and immediately loads will produce an empty canvas and blame the media. Wait for `AsyncVideoLoader: Worker thread running` in the journal before the first load (60 s is a safe bound); a long-running compositor logged it long ago and is obviously ready. Cost one INVALID rung to learn, 2026-08-31.
- **The hang guard can refuse a layer (G12, `feat/per-surface-pacing`).** On `fp530-8gb` — autodetected from PCI `1002:15d8` plus RAM — the compositor **caps concurrent 4K-class decode sessions at 4** and refuses the 5th, because 5 and beyond is the measured `vcn_dec` ring-hang region and a hang takes all four outputs with it. Everything you need to know when a layer mysteriously does not play:
  - The refusal is at **reveal** (`/mtcfollow`), not at load. A refused layer stays loaded and holding its frame; the project keeps running. Grep `HangGuard: REFUSED` — it names the cue and the count.
  - It counts **actively decoding** sessions, not armed ones, so an ordinary chained show that arms its whole cue chain is never refused. Loading over an existing layer is **net-zero** and can never be refused (that would put the previous cue's media on a live output).
  - **Shared layers are one session, not N**: only the decode driver reserves, and driver promotion transfers the slot.
  - `HangGuard: ADVISORY armed4k=N cap=4` at load means *more is armed than can decode at once* — exposure, not an error, and normal on a chained show.
  - Every other machine is **monitor-only** and refuses nothing. The startup line says which you have, and names the source of each setting (`compiled` vs `flag`).
  - Escape hatch for capacity measurement: `--hang-guard=off` via `OPERATOR_FLAGS` in `/etc/cuems/videocomposer-flags.env`. Read once at startup, so it costs a restart — **pre-show only**. `--saturation-monitor=off` exists too, purely so the monitor's own cost can be A/B'd against the same binary.
  - Known bypasses, by design and stated at startup: a second videocomposer, the separate `cuems-videoindexer` process, and HAP layers (which decode on the CPU and open no VCN session). The ledger is per-process; the hang is per-GPU.
  Plan + drill data: `cuems-RELATIONS/Plans/2026-08-31-vc-saturation-warn-and-hang-guard.md`, `baselines/fp530-capacity/results/g12-drills-20260901/`. ClickUp 869en65tm.
